/**
 * build-system-design-stripe-film.mjs
 *
 ...
 *
 * Output: public/system-design-lessons/design-stripe.mp4
 */
...
const FPS = 24;
const OUT_DIR = path.resolve('scratchpad/film_render_sd_stripe');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-stripe.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-stripe/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-stripe-intro': {
        tag: '⚡ ACCIDENTAL DOUBLE-CHARGE: NETWORK TIMEOUT & RETRY SPIKES',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-stripe-idempotency': {
        tag: '🔒 IDEMPOTENCY ENGINE: REDIS REDLOCK & ATOMIC KEY DEDUPING',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-stripe-saga': {
        tag: '🔄 SAGA ORCHESTRATION: COMPENSATING ROLLBACKS VS 2PC LOCKS',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-stripe-outbox-cdc': {
        tag: '📜 TRANSACTIONAL OUTBOX: DEBEZIUM WAL TAILING PREVENTS DUAL-WRITE FAILS',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-stripe-double-entry-ledger': {
        tag: '💰 DOUBLE-ENTRY LEDGER: APPEND-ONLY IMMUTABLE BALANCES',
        color: '#f472b6',
...
async function assembleFilm() {
    console.log('🎬 Assembling Gemini Omni AI Video for Design Stripe...\n');
    ...
    for (const beat of SYSTEM_DESIGN_STRIPE_BEATS) {
        ...
        const omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-stripe--${beat.id}.mp4`);
    ...
    console.log(`\n🎉 Design Stripe System Design Video Successfully Compiled!`);
