/**
 * generate-fresh-4-and-5-and-concat.mjs
 *
 * Full pipeline for generating fresh Clip 4 and Clip 5 in new Gemini chats,
 * cropping watermarks, and concatenating all 6 clips into the master film.
 */
...
async function runPipeline() {
    console.log("========================================================");
    console.log("🎬 WHATSAPP SYSTEM DESIGN MASTER VIDEO GENERATION");
    console.log("========================================================\n");

    // --- STEP 1: Wait for Clip 4 rendering in current tab & download ---
    console.log("⏳ Step 1: Waiting 45 seconds for Clip 4 video rendering...");
    await new Promise(r => setTimeout(r, 45000));

    console.log("🔄 Step 2: Refreshing tab to reveal Clip 4 download link...");
    const refresh1 = `
...
`;
        ...
        if (dlFile4 && fs.existsSync(dlFile4)) {
            fs.copyFileSync(dlFile4, path.join(CLIPS_DIR, 'whatsapp-raw-4-fixed.mp4'));
            console.log("✅ Saved raw Clip 4 fixed!");
        }
    }
...
`;
        ...
        if (dlFile5 && fs.existsSync(dlFile5)) {
            fs.copyFileSync(dlFile5, path.join(CLIPS_DIR, 'whatsapp-raw-5-fixed.mp4'));
            console.log("✅ Saved raw Clip 5 fixed!");
        }
    }
    ...
    for (let i = 0; i < rawClipsList.length; i++) {
        ...
        if (!fs.existsSync(raw)) {
            console.warn(`⚠️ Warning: Clip file ${raw} not found, checking fallback...`);
            continue;
        }
    ...
    console.log(`\n========================================================`);
    console.log(`🎬 Concatenating ${cleanPaths.length} Unique Clips with xfade Dissolve Transitions...`);
    console.log(`========================================================`);
