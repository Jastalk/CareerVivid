/**
 * build-system-design-youtube-film.mjs
 *
 * Assembles the beginner-friendly, hilarious, high-converting YouTube Explainer Gemini Omni AI Video
 * for System Design: How to Design YouTube (Global Video Streaming & CDN).
 * Specs:
 *   1. 50-Pound Birthday Cake Envelope & 4K Streaming Fail.
 *   2. Slicing Film Reel into 2-Second Bread Slice Chunks (.ts) across resolutions.
 *   3. Elevator ABR Adaptive Bitrate (1080p -> 360p) seamless switching.
 *   4. Neighborhood Convenience Store CDN Edge PoP Delivery.
 *   5. Hot Edge CDN vs Cold Blob Storage Vault Archiving.
 *   6. Non-looping smooth video speed stretch (setpts) + Chirp3-HD Fenrir voiceover + soft BGM fade out.
 *
 * Output: public/system-design-lessons/design-youtube.mp4
 */
...
const OUT_DIR = path.resolve('scratchpad/film_render_sd_youtube');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-youtube.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-youtube/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-youtube-intro': {
        tag: '🎂 50-POUND CAKE IN ENVELOPE: RAW 4K MONOLITH FAIL',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-youtube-transcoding-hls': {
        tag: '🍞 BREAD SLICER PIPELINE: 2s HLS CHUNKS (.ts)',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-youtube-adaptive-bitrate': {
        tag: '📶 ELEVATOR ABR: SEAMLESS 1080p ➔ 360p AUTO SWITCH',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-youtube-cdn-edges': {
        tag: '🏪 CONVENIENCE STORE CDN: LOCAL PoP EDGE DELIVERY (<10ms)',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-youtube-blob-cold-storage': {
        tag: '🧊 COLD VAULT ARCHIVE: HOT CDN vs CHEAP BLOB STORAGE',
        color: '#f472b6',
...
const CLIP_FALLBACKS = {
    'sd-youtube-cold-storage': path.resolve('public/system-design-lessons/clips/sd-youtube--sd-youtube-blob-cold-storage.mp4'),
    'sd-youtube-call-to-action': path.resolve('public/system-design-lessons/clips/sd-youtube--sd-youtube-outro.mp4'),
};

async function assembleYouTubeFilm() {
    console.log('🎬 Assembling Humorous & High-Converting Gemini Omni AI Video for Design YouTube...\n');
    ...
    for (const beat of SYSTEM_DESIGN_YOUTUBE_BEATS) {
        ...
        let omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-youtube--${beat.id}.mp4`);
    ...
    console.log('\n🎞️ Concatenating all Design YouTube beat MP4 files into continuous film...');
    ...
    console.log(`\n🎉 Design YouTube Gemini Omni AI Video Successfully Compiled!`);
...
assembleYouTubeFilm().catch(console.error);
