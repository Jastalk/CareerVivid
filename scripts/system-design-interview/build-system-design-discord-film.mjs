/**
 * build-system-design-discord-film.mjs
 *
 ...
 *
 * Output: public/system-design-lessons/design-discord.mp4
 */
...
const FPS = 24;
const OUT_DIR = path.resolve('scratchpad/film_render_sd_discord');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-discord.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-discord/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-discord-intro': {
        tag: '⚡ 200M USERS: RELATIONAL DB CONTENTION OVERLOAD',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-discord-actor-model': {
        tag: '🎭 BEAM ACTOR MODEL: 2KB LIGHTWEIGHT PROCESS PER CHANNEL',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-discord-scylladb-store': {
        tag: '💾 SCYLLADB MESSAGE STORE: SNOWFLAKE CLUSTERING KEYS',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-discord-sfu-voice': {
        tag: '🎙️ WEBRTC SFU: ROUTING OPUS AUDIO PACKETS (-90% BANDWIDTH)',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-discord-read-pointers': {
        tag: '🔴 READ POINTER CRDTS: LOCAL TRACKING PREVENTS WRITE SPIKES',
        color: '#f472b6',
...
async function assembleFilm() {
    console.log('🎬 Assembling Gemini Omni AI Video for Design Discord...\n');
    ...
    for (const beat of SYSTEM_DESIGN_DISCORD_BEATS) {
        ...
        const omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-discord--${beat.id}.mp4`);
    ...
    console.log(`\n🎉 Design Discord System Design Video Successfully Compiled!`);
