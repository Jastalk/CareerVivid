/**
 * build-system-design-dropbox-film.mjs
 *
 ...
 *
 * Output: public/system-design-lessons/design-dropbox.mp4
 */
...
const FPS = 24;
const OUT_DIR = path.resolve('scratchpad/film_render_sd_dropbox');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-dropbox.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-dropbox/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-dropbox-intro': {
        tag: '⚡ FULL FILE RE-UPLOAD: BANDWIDTH CHOKE & SYNCHRONIZATION LAG',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-dropbox-cdc': {
        tag: '✂️ RABIN FINGERPRINTING: 4MB CONTENT-DEFINED CHUNKING',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-dropbox-dedup': {
        tag: '🔑 SHA-256 DEDUP: ZERO-BYTE UPLOADS FOR DUPLICATE CHUNKS',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-dropbox-sync-engine': {
        tag: '🔄 DELTA SYNC ENGINE: METADATA COMMITS & LONG-POLLING QUEUES',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-dropbox-conflict-resolution': {
        tag: '🛡️ VECTOR CLOCKS: CONFLICT COPIES & RESUMABLE MULTIPART UPLOADS',
        color: '#f472b6',
...
async function assembleFilm() {
    console.log('🎬 Assembling Gemini Omni AI Video for Design Dropbox...\n');
    ...
    for (const beat of SYSTEM_DESIGN_DROPBOX_BEATS) {
        ...
        const omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-dropbox--${beat.id}.mp4`);
    ...
    console.log(`\n🎉 Design Dropbox System Design Video Successfully Compiled!`);
