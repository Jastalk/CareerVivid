/**
 * generate-sd-api-narration.mjs
 *
 * Synthesizes Chirp3-HD-Fenrir English voiceover narration clips
 * for System Design: APIs and Data Models lesson.
 *
 * Output: public/assets/ccaf-narration/sd-api-data/en/chirp-fenrir/<beatId>.wav
 */
...
const OUT_DIR = 'public/assets/ccaf-narration/sd-api-data/en/chirp-fenrir';
...
async function generateSdApiNarration() {
    console.log('⚡ Synthesizing System Design Chirp3-HD Fenrir Narration Clips...\n');
    ...
    for (const beat of SYSTEM_DESIGN_API_BEATS) {
        ...
        try {
            ...
            console.log(`   ✅ ${beat.id.padEnd(25)} -> ${duration}s (${(wavBuf.length / 1024).toFixed(1)} KB)`);
        } catch (err) {
            console.error(`   ❌ Failed ${beat.id}:`, err.message);
        }
    }
    console.log('\n🎉 System Design Audio Synthesis Complete!');
}

generateSdApiNarration();
