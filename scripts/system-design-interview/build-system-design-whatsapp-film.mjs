/**
 * build-system-design-whatsapp-film.mjs
 *
 * Assembles the beginner-friendly, hilarious, high-converting YouTube Explainer Gemini Omni AI Video
 * for System Design: How to Design WhatsApp / Discord (Real-Time Messaging & WebSockets).
 * Built with clips generated by NEW model: veo-3.1-lite-generate-001 ($0.40/clip).
 * Specs:
 *   1. The Double Check Mark Anxiety (Delivering 100B daily texts with zero drop).
 *   2. Persistent WebSockets (Stateful connection gateways with sub-50ms push).
 *   3. Group Chat Fan-Out (Asynchronous message queues to 200 group members).
 *   4. End-to-End Encryption (Signal Protocol device-level key exchange).
 *   5. Presence Service (Redis Pub/Sub online/offline pings).
 *   6. Non-looping smooth video speed stretch (setpts) + Chirp3-HD Fenrir voiceover + soft BGM fade out.
 *
 * Output: public/system-design-lessons/design-whatsapp.mp4
 */
...
const OUT_DIR = path.resolve('scratchpad/film_render_sd_whatsapp');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-whatsapp.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-whatsapp/en/chirp-fenrir');
...
// Selective concept badges for WhatsApp System Design
const SELECTIVE_BADGES = {
    'sd-whatsapp-intro': {
        tag: '💬 REALTIME MESSAGING: 100B+ DAILY MESSAGES (<50MS)',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-whatsapp-websockets': {
        tag: '⚡ WEBSOCKET POOL: STATEFUL DUPLEX GATEWAY FLEETS',
        color: '#38bdf8',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#0284c7',
    },
    'sd-whatsapp-outbox-pattern': {
        tag: '🔄 OUTBOX & CDC: TRANSACTIONAL DEBEZIUM WAL TAILING',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-whatsapp-saga-delivery': {
        tag: '🛡️ SAGA PATTERN: EXACTLY-ONCE IDEMPOTENT RETRY QUEUES',
        color: '#a7f3d0',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#059669',
    },
    'sd-whatsapp-encryption': {
        tag: '🔐 E2E ENCRYPTION: SIGNAL PROTOCOL DOUBLE RATCHET',
        color: '#f87171',
...
function buildHTML(beat, progressPercent, durationSec) {
    // Captions are rendered from the script's own narration text —
    // never reworded, only chunked and timed.
        ...
        <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .progress-bar {
                ...
                background: linear-gradient(90deg, #10b981, #38bdf8, #f59e0b, #ec4899);
...
const CLIP_FALLBACKS = {
    'sd-whatsapp-outbox-pattern': path.resolve('public/system-design-lessons/clips/sd-whatsapp--sd-whatsapp-group-fanout.mp4'),
    'sd-whatsapp-saga-delivery': path.resolve('public/system-design-lessons/clips/sd-whatsapp--sd-whatsapp-presence.mp4'),
    'sd-whatsapp-call-to-action': path.resolve('public/system-design-lessons/clips/sd-whatsapp--sd-whatsapp-outro.mp4'),
};

async function assembleWhatsAppFilm() {
    console.log('🎬 Assembling Humorous & High-Converting Gemini Omni AI Video for Design WhatsApp (Veo 3.1 Lite)...\n');
    ...
    for (const beat of SYSTEM_DESIGN_WHATSAPP_BEATS) {
        ...
        let omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-whatsapp--${beat.id}.mp4`);
    ...
    for (let i = 0; i < beatsToProcess.length; i++) {
        ...
        // Playwright pauses every CSS animation and seeks it per frame, so the
        // render is deterministic — same beat, same frames, every run.
        ...
        // True speed, looped — never stretched. setpts at 1.9x is visible as
        // slow motion; a boomerang plays at real speed and its join is a
        // repeated frame, so looping it has no cut. boomerangBed also scales to
        // 1080p and trims 4% off each edge, which removes the pseudo-Latin Veo
        // writes along the border.
        const bed = omniVideoPath
            ? boomerangBed(omniVideoPath, path.join(OUT_DIR, 'beds', `${beat.id}.mp4`))
            : null;
        // A slow pan across the beat, so repeated passes are never framed alike.
    ...
    console.log('\n🎞️ Concatenating all Design WhatsApp beat MP4 files into continuous film...');
    ...
    console.log(`\n🎉 Design WhatsApp Veo 3.1 Lite AI Video Successfully Compiled!`);
...
assembleWhatsAppFilm().catch(console.error);
