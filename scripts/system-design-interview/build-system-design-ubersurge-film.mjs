/**
 * build-system-design-ubersurge-film.mjs
 *
 * Assembles 1080p YouTube Explainer Gemini Omni AI Video for:
 *   System Design: How to Design Uber's Dynamic Surge Pricing Engine & Real-Time Heatmaps
 *
 * Output: public/system-design-lessons/design-uber-surge.mp4
 */
...
const FPS = 24;
const OUT_DIR = path.resolve('scratchpad/film_render_sd_ubersurge');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-uber-surge.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-ubersurge/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-ubersurge-intro': {
        tag: '⚡ 50K RIDERS VS 500 DRIVERS: DYNAMIC SURGE PRICING',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-ubersurge-h3-hexagons': {
        tag: '🔷 UBER H3 RES 8: 0.7 KM² UNIFORM HEXAGON SPATIAL BUCKETS',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-ubersurge-flink-stream': {
        tag: '🚀 APACHE FLINK: 10S SLIDING WINDOW SUPPLY-DEMAND RATIOS',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-ubersurge-price-quote': {
        tag: '🔒 QUOTE LOCKING: 5-MIN SIGNED FARE CONSISTENCY & REDIS LOCKS',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-ubersurge-anti-fraud': {
        tag: '🚨 ANTI-FRAUD: ANOMALY DETECTION BLOCKS GPS MANIPULATION',
        color: '#f472b6',
...
async function assembleFilm() {
    console.log('🎬 Assembling Gemini Omni AI Video for Design Uber Surge Pricing...\n');
    ...
    for (const beat of SYSTEM_DESIGN_UBER_SURGE_BEATS) {
        ...
        const omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-ubersurge--${beat.id}.mp4`);
    ...
    console.log(`\n🎉 Design Uber Surge Pricing Video Successfully Compiled!`);
