/**
 * build-system-design-netflix-film.mjs
 *
 ...
 *
 * Output: public/system-design-lessons/design-netflix.mp4
 */
...
const FPS = 24;
const OUT_DIR = path.resolve('scratchpad/film_render_sd_netflix');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-netflix.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-netflix/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-netflix-intro': {
        tag: '⚡ 250M SUBSCRIBERS: CLOUD ORIGIN BACKBONE BOTTLENECK',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-netflix-transcoding': {
        tag: '🎬 CHUNK TRANSCODING: 2-SECOND PARALLEL WORKER SLICES',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-netflix-abr': {
        tag: '📶 ABR MANIFESTS: SEAMLESS HLS/DASH BITRATE SWITCHING',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-netflix-open-connect': {
        tag: '🌐 OPEN CONNECT CDN: ISP EDGE PRE-POSITIONING (-95% ORIGIN TRAFFIC)',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-netflix-stampede-resilience': {
        tag: '🛡️ CACHE COALESCING: STAMPEDE PREVENTION & BGP FAILOVER',
        color: '#f472b6',
...
async function assembleFilm() {
    console.log('🎬 Assembling Gemini Omni AI Video for Design Netflix...\n');
    ...
    for (const beat of SYSTEM_DESIGN_NETFLIX_BEATS) {
        ...
        const omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-netflix--${beat.id}.mp4`);
    ...
    console.log(`\n🎉 Design Netflix System Design Video Successfully Compiled!`);
