/**
 * build-system-design-uber-film.mjs
 *
 * Assembles the beginner-friendly, hilarious, high-converting YouTube Explainer Gemini Omni AI Video
 * for System Design: How to Design Uber (Real-Time Location Tracking & Matching).
 * Specs:
 *   1. Midnight Rainstorm & 5 Million Drivers O(N) Scanning Fail.
 *   2. Earth Watermelon Sliced into H3 Hexagonal Spatial Cells.
 *   3. 1.25 Million GPS Pings/Sec In-Memory Redis RAM Savior.
 *   4. Two-Phase Distributed Mutex Lock (10s Window) to prevent double booking.
 *   5. Real-Time Demand-Supply Surge Pricing ($10 -> $40) Multiplier Engine.
 *   6. Non-looping smooth video speed stretch (setpts) + Chirp3-HD Fenrir voiceover + soft BGM fade out.
 *
 * Output: public/system-design-lessons/design-uber.mp4
 */
...
const OUT_DIR = path.resolve('scratchpad/film_render_sd_uber');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-uber.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-uber/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-uber-intro': {
        tag: '🌧️ 1:00 AM RAINSTORM: 5 MILLION DRIVERS O(N) SCAN FAIL',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-uber-spatial-grid': {
        tag: '🍉 EARTH SLICED LIKE WATERMELON: H3 HEXAGONAL GRID (O(1))',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-uber-gps-avalanche': {
        tag: '⚡ 1.25 MILLION PINGS/SEC: REDIS RAM SAVES DATABASE DISK',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-uber-matching-lock': {
        tag: '🔒 DISTRIBUTED LOCK: 10s DRIVER MUTEX (NO DOUBLE BOOKING)',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-uber-surge-pricing': {
        tag: '📈 SURGE ENGINE: $10 ➔ $40 MULTIPLIER ATTRACTS DRIVER SWARM',
        color: '#f472b6',
...
const CLIP_FALLBACKS = {
    'sd-uber-call-to-action': path.resolve('public/system-design-lessons/clips/sd-uber--sd-uber-outro.mp4'),
};

async function assembleUberFilm() {
    console.log('🎬 Assembling Humorous & High-Converting Gemini Omni AI Video for Design Uber...\n');
    ...
    for (const beat of SYSTEM_DESIGN_UBER_BEATS) {
        ...
        let omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-uber--${beat.id}.mp4`);
    ...
    console.log('\n🎞️ Concatenating all Design Uber beat MP4 files into continuous film...');
    ...
    console.log(`\n🎉 Design Uber Gemini Omni AI Video Successfully Compiled!`);
...
assembleUberFilm().catch(console.error);
