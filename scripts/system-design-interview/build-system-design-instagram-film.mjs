/**
 * build-system-design-instagram-film.mjs
 *
 * Assembles the beginner-friendly, hilarious, high-converting YouTube Explainer Gemini Omni AI Video
 * for System Design: How to Design Instagram (Global News Feed & Image Processing).
 * Specs:
 *   1. Celebrity Post Overload (300M Mailboxes Write Amplification Fail).
 *   2. Hybrid Fan-Out Architecture (Mailboxes for Users + Megaphone for Celebrities).
 *   3. Async Image Resizing Factory (20 MB iPhone photo sliced into 3 sizes).
 *   4. Sharded Redis Feed Cache (Pre-computed microsecond ZSET timeline lists).
 *   5. Blob Storage + Global Edge CDN Acceleration.
 *   6. Non-looping smooth video speed stretch (setpts) + Chirp3-HD Fenrir voiceover + soft BGM fade out.
 *
 * Output: public/system-design-lessons/design-instagram.mp4
 */
...
const OUT_DIR = path.resolve('scratchpad/film_render_sd_instagram');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-instagram.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-instagram/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-instagram-intro': {
        tag: '📸 CELEBRITY POST OVERLOAD: 300M MAILBOXES FAN-OUT FAIL',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-instagram-fanout-hybrid': {
        tag: '📣 HYBRID FAN-OUT: MAILBOXES FOR USERS + MEGAPHONE FOR CELEBS',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-instagram-image-pipeline': {
        tag: '🏭 PHOTO FACTORY: 20MB IMAGE CUT INTO 3 FAST SIZES',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-instagram-redis-feed': {
        tag: '⚡ SHARDED REDIS RAM: PRE-SORTED TIMELINE POST IDs (<1ms)',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-instagram-storage-cdn': {
        tag: '📦 BLOB STORAGE & CDN: CHEAP OBJECT VAULT + LOCAL EDGE',
        color: '#f472b6',
...
const CLIP_FALLBACKS = {
    'sd-instagram-hot-cdn': path.resolve('public/system-design-lessons/clips/sd-instagram--sd-instagram-storage-cdn.mp4'),
    'sd-instagram-call-to-action': path.resolve('public/system-design-lessons/clips/sd-instagram--sd-instagram-outro.mp4'),
};

async function assembleInstagramFilm() {
    console.log('🎬 Assembling Humorous & High-Converting Gemini Omni AI Video for Design Instagram...\n');
    ...
    for (const beat of SYSTEM_DESIGN_INSTAGRAM_BEATS) {
        ...
        let omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-instagram--${beat.id}.mp4`);
    ...
    console.log('\n🎞️ Concatenating all Design Instagram beat MP4 files into continuous film...');
    ...
    console.log(`\n🎉 Design Instagram Gemini Omni AI Video Successfully Compiled!`);
...
assembleInstagramFilm().catch(console.error);
