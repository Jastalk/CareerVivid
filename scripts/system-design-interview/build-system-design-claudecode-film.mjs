/**
 * build-system-design-claudecode-film.mjs
 *
 * Assembles the beginner-friendly, hilarious, high-converting YouTube Explainer Gemini Omni AI Video
 * for System Design: How to Design Claude Code (Agentic AI System & Subagent Fleet).
 * Specs:
 *   1. One-Man Band 100k Line Codebase Overload.
 *   2. General Manager & Specialist Subagent Fleet (Researcher, Debugger, Auditor).
 *   3. Context Window Sliding & Markdown Summary Checkpoints.
 *   4. Runtime Permission Interception (Blocking rm -rf / & DB drops).
 *   5. Event-Driven Reactive Loop (Sleep during execution + instant callback wakeup).
 *   6. Non-looping smooth video speed stretch (setpts) + Chirp3-HD Fenrir voiceover + soft BGM fade out.
 *
 * Output: public/system-design-lessons/design-claude-code.mp4
 */
...
const OUT_DIR = path.resolve('scratchpad/film_render_sd_claudecode');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-claude-code.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-claudecode/en/chirp-fenrir');
...
const SELECTIVE_BADGES = {
    'sd-claudecode-intro': {
        tag: '🤯 100K CODE LINES: SINGLE-PROMPT CONTEXT OVERFLOW',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-claudecode-subagent-fleet': {
        tag: '🤝 MANAGER & SPECIALISTS: CONCURRENT SUBAGENT FLEET',
        color: '#34d399',
        bg: 'rgba(6, 78, 59, 0.75)',
        border: '#10b981',
    },
    'sd-claudecode-context-window': {
        tag: '📝 CONTEXT COMPRESSION: SUMMARY CHECKPOINTS (<10K TOKENS)',
        color: '#7dd3fc',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#38bdf8',
    },
    'sd-claudecode-safety-guards': {
        tag: '🛡️ SAFETY BOUNCER: PERMISSION HOOKS BLOCK DESTRUCTIVE COMMANDS',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-claudecode-event-loop': {
        tag: '⚡ REACTIVE LOOP: ZERO CPU POLLING + EVENT CALLBACK WAKEUP',
        color: '#f472b6',
...
const CLIP_FALLBACKS = {
    'sd-claudecode-reactive-loop': path.resolve('public/system-design-lessons/clips/sd-claudecode--sd-claudecode-event-loop.mp4'),
    'sd-claudecode-failure-modes': path.resolve('public/system-design-lessons/clips/sd-claudecode--sd-claudecode-intro.mp4'),
    'sd-claudecode-benchmark': path.resolve('public/system-design-lessons/clips/sd-claudecode--sd-claudecode-subagent-fleet.mp4'),
    'sd-claudecode-call-to-action': path.resolve('public/system-design-lessons/clips/sd-claudecode--sd-claudecode-outro.mp4'),
};

async function assembleClaudeCodeFilm() {
    console.log('🎬 Assembling Humorous & High-Converting Gemini Omni AI Video for Design Claude Code...\n');
    ...
    for (const beat of SYSTEM_DESIGN_CLAUDE_CODE_BEATS) {
        ...
        let omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-claudecode--${beat.id}.mp4`);
    ...
    console.log('\n🎞️ Concatenating all Design Claude Code beat MP4 files into continuous film...');
    ...
    console.log(`\n🎉 Design Claude Code Gemini Omni AI Video Successfully Compiled!`);
...
assembleClaudeCodeFilm().catch(console.error);
