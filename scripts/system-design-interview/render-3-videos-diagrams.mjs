/**
 * render-3-videos-diagrams.mjs
 *
 * Programmatic HTML/CSS/SVG progressive diagram renderer for Beats 2-7 across 3 videos:
 ...
 *
 * Fixed UI Layout:
 *   - Metrics Rail relocated to top-left below top bar (top: 115px) to eliminate overlap with bottom captions.
 *   - Karaoke captions isolated at bottom: 55px with z-index: 100 and clean text shadows (Zero flashing/overlap).
 */
...
import { karaokeChunks, karaokeHTML, KARAOKE_CSS, mark } from './karaokeSubtitles.mjs';

const FPS = 30;

const VIDEO_CONFIGS = [
    { id: 'sd-vector-rag', beats: VECTOR_RAG_BEATS, title: 'Vector Index Sharding & Hybrid RAG' },
    { id: 'sd-agent-swarms', beats: AGENT_SWARMS_BEATS, title: 'AI Agent Orchestration & Subagent Swarms' },
    { id: 'sd-gpu-fleet', beats: GPU_FLEET_BEATS, title: 'GPU Fleet Scheduling & Multi-Tenant K8s' },
];
...
function buildDiagramHTML(beat, durationSec, currentTimeMs, videoTitle) {
    ...
    for (const edge of spec.edges) {
        ...
        ${progress > 0.5 ? `
        <rect x="${midX - 115}" y="${midY - 16}" width="230" height="32" rx="16" fill="rgba(15, 23, 42, 0.95)" stroke="#38bdf8" stroke-width="1.5"/>
...
async function renderDiagrams() {
    console.log('🎨 Programmatic Diagram Renderer starting for 3 Videos (Beats 2-7)...\n');
    ...
    await browser.close();
    console.log('🎉 Programmatic Diagram Rendering Complete for All 3 Videos!');
}
