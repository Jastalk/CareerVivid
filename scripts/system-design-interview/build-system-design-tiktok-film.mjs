/**
 * build-system-design-tiktok-film.mjs
 *
 * Assembles the beginner-friendly, hilarious, high-converting YouTube Explainer Gemini Omni AI Video
 * for System Design: How to Design TikTok (High-Throughput Recommendation & Real-time Video Feed).
 * Specs:
 *   1. The Infinite Scroll Addiction (Serving 1 Billion active swiping users with <100ms latency).
 *   2. The Recommendation Pipeline (Recall candidate pool ➔ Neural Ranking Model ➔ Filter).
 *   3. Viral Counter Sharding (Handling 10 million 'Likes' per second without database lockup).
 *   4. Instant Play Prefetching (Edge CDN pre-buffering chunk 1 before your thumb even finishes swiping).
 *   5. Multi-Bitrate Storage & CDN (Transcoding raw videos into adaptive HLS variants).
 *   6. Non-looping smooth video speed stretch (setpts) + Chirp3-HD Fenrir voiceover + soft BGM fade out.
 *
 * Output: public/system-design-lessons/design-tiktok.mp4
 */
...
import { SYSTEM_DESIGN_TIKTOK_BEATS } from './systemDesignTikTokScript.ts';
...
const OUT_DIR = path.resolve('scratchpad/film_render_sd_tiktok');
const FINAL_MP4 = path.resolve('public/system-design-lessons/design-tiktok.mp4');
const NARRATION_DIR = path.resolve('public/assets/system-design-narration/sd-tiktok/en/chirp-fenrir');
...
// Selective concept badges for TikTok System Design
const SELECTIVE_BADGES = {
    'sd-tiktok-intro': {
        tag: '📱 INFINITE SCROLL: PRE-COMPUTED FEED (<100MS LATENCY)',
        color: '#38bdf8',
        bg: 'rgba(12, 74, 110, 0.75)',
        border: '#0284c7',
    },
    'sd-tiktok-recommendation': {
        tag: '🤖 RECOMMENDATION ENGINE: VECTOR RECALL ➔ ML RANKING',
        color: '#f472b6',
        bg: 'rgba(131, 24, 67, 0.75)',
        border: '#ec4899',
    },
    'sd-tiktok-counter-sharding': {
        tag: '🔥 VIRAL LIKES: N-SHARD DISTRIBUTED REDIS COUNTERS',
        color: '#f87171',
        bg: 'rgba(127, 29, 29, 0.75)',
        border: '#ef4444',
    },
    'sd-tiktok-cdn-prefetch': {
        tag: '⚡ ZERO-LAG SWIPE: EDGE CDN PREFETCHING (CHUNK 1)',
        color: '#fde047',
        bg: 'rgba(113, 63, 18, 0.75)',
        border: '#eab308',
    },
    'sd-tiktok-storage-hybrid': {
        tag: '🎬 ADAPTIVE STREAMING: MULTI-BITRATE HLS & COLD STORAGE',
        color: '#34d399',
        ...
        <style>
            * { box-sizing: border-box; margin: 0; padding: 0; }
            ...
            .progress-bar {
                ...
                background: linear-gradient(90deg, #ec4899, #38bdf8, #10b981, #f59e0b);
...
const CLIP_FALLBACKS = {
    'sd-tiktok-transcoding': path.resolve('public/system-design-lessons/clips/sd-tiktok--sd-tiktok-cdn-prefetch.mp4'),
    'sd-tiktok-call-to-action': path.resolve('public/system-design-lessons/clips/sd-tiktok--sd-tiktok-intro.mp4'),
};

async function assembleTikTokFilm() {
    console.log('🎬 Assembling Humorous & High-Converting Gemini Omni AI Video for Design TikTok...\n');

    const beatsToProcess = [];
    let totalSeconds = 0;

    for (const beat of SYSTEM_DESIGN_TIKTOK_BEATS) {
        const audioPath = path.join(NARRATION_DIR, `${beat.id}.wav`);
        let omniVideoFile = path.join(OMNI_CLIPS_DIR, `sd-tiktok--${beat.id}.mp4`);
    ...
    console.log('\n🎞️ Concatenating all Design TikTok beat MP4 files into continuous film...');
    ...
    console.log(`\n🎉 Design TikTok Gemini Omni AI Video Successfully Compiled!`);
...
assembleTikTokFilm().catch(console.error);
